function [Collect_LP]=Panel_LP_IV_withFWDControls(YY,XX,FWDControls,Controls,...
    DetermControls,Instr,a_id,a_time,settingsLP, interaction)
...
%           FWDControls: TxNxKf 3D matrix of forward controls (panel),
%                        included horizon-by-horizon in cumulative-change form:
%                        control_{t+h} - control_{t-1}
...
%
% Notes:    Relative to Panel_LP_IV, this version additionally includes
%           FWDControls as horizon-specific forward controls, constructed as
%           control_{t+h} - control_{t-1} for each horizon h.
...
for hh = WhenStartIRF:maxH
            ...
            for pick = 1:size(FWDControls,3)
                pickControl = DemeanVariables(FWDControls(:,:,pick));
                xx_tplus_h = mat_single_leadlag(pickControl,hh) - mat_single_leadlag(pickControl,-1);  
                vec_Controls_t = [vec_Controls_t xx_tplus_h(:) ];
            end 
