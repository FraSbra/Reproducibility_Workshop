%% Figure 3: Counterfactual --> no MP response
close all

FWDControls = cat(3, ...                               
    repmat(100 .* log(other.DEXUSEU),1,No_Countries), ...
    repmat(eonia,1,No_Countries) ...
    );
...
% === SAVE FIGURE AS PNG ===
savePath = fullfile(saveResults_fig, 'counter_no_MP.png');
