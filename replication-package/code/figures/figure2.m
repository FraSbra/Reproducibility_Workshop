%% Figure 2: IRF of downside risk to US MP shock
close all 
...
% === SAVE FIGURE AS PNG ===
savePath = fullfile(saveResults_fig, 'linear_downside_risk_irf.png');
...
ub68_fed = irf_fed + 1.00 * se_fed;  lb68_fed = irf_fed - 1.00 * se_fed;
ub90_fed = irf_fed + 1.645 * se_fed; lb90_fed = irf_fed - 1.645 * se_fed;
...
% 90% bands
fill(x_fill, [ub90_fed; flipud(lb90_fed)], [0.60 0.75 1.00], 'EdgeColor', 'none', 'FaceAlpha', 0.35, 'HandleVisibility', 'off');
fill(x_fill, [ub90_2; flipud(lb90_2)], [1.00 0.75 0.60], 'EdgeColor', 'none', 'FaceAlpha', 0.35, 'HandleVisibility', 'off');

% 68% bands
fill(x_fill, [ub68_fed; flipud(lb68_fed)], [0.20 0.50 0.90], 'EdgeColor', 'none', 'FaceAlpha', 0.40, 'HandleVisibility', 'off');
fill(x_fill, [ub68_2; flipud(lb68_2)], [0.90 0.40 0.20], 'EdgeColor', 'none', 'FaceAlpha', 0.40, 'HandleVisibility', 'off');

% IRF lines
plot(h, irf_fed, 'b-', 'LineWidth', 2, 'DisplayName', 'Downside Risk');
...
% === SAVE ===
savePath = fullfile(saveResults_fig, 'comparison_downside_risk_vs_avg_irf.png');
set(gcf, 'PaperPositionMode', 'auto');
print(gcf, savePath, '-dpng', '-r300');
